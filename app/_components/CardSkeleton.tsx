type CardSkeletonProps = {
  className?: string;
  compact?: boolean; // remove outer margin/padding when true
};

export function SharedCardSkeleton({ className = "" }: { className?: string }) {
  return (
    <div
      className={`w-[min(36vw,130px)] sm:w-56 md:w-60 lg:w-64 aspect-16/23 rounded-xl border-2 sm:border-4 border-gray-300 p-4 flex flex-col justify-between overflow-hidden relative bg-linear-to-r from-gray-200 via-gray-100 to-gray-200 animate-shimmer ${className}`}
      style={{
...
export function CardSkeleton({ className = "", compact = false }: CardSkeletonProps) {
  // Kita buat 5 item untuk menampung batas maksimal di desktop
...
export function CardHeaderSkeleton() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-4 md:gap-6 px-2 sm:px-4 md:px-6 my-6 md:my-8">
      {[...Array(5)].map((_, index) => (
        <SharedCardSkeleton
          key={index}
...
export function CardTileSkeleton({ className = "" }: CardSkeletonProps) {
  return (
    <div
      className={`aspect-3/4 w-full rounded-xl overflow-hidden bg-linear-to-r from-gray-200 via-gray-100 to-gray-200 animate-shimmer border-2 border-gray-300 ${className}`}
      style={{
        backgroundSize: "200% 100%",
      }}
    />
...
export function CardImageSkeleton({ className = "" }: CardSkeletonProps) {
  return (
    <div
      className={`absolute inset-0 z-10 rounded-xl bg-linear-to-r from-gray-200 via-gray-100 to-gray-200 animate-shimmer ${className}`}
      style={{
        backgroundSize: "200% 100%",
      }}
    />
